...
def load_Song_Stats(dataJSON, speed, key, retest=False, versionNum=-1):
    ...
    AiJSON = {}
    try:
        with open(f"_AIcache/{hash}/{diffNum} {speed}.json",
                  encoding='ISO-8859-1') as score_json:  # Checkes if the result has already been calculated before
        ...
        try:
            ...
            if retest and cacheVNum != versionNum:
                ...
                (mapData, charIndex) = setup.loadMapData(key, diffNum, False, True)
                ...
                for i in range(0, len(beatmapSets)):
                    if beatmapSets[i]["_difficultyRank"] == diffNum:
                        ...
                        break
                AiJSON['lackStats'] = tech_calc.mapCalculation(mapData, bpm, njs, False, False)
...
def newPlayerStats(userID, scoreCount, retest=False, versionNum=-1):
    ...
    for i in range(0, len(playerJSON['data'])):
        if playerJSON['data'][i]['pp'] != 0:
            ...
            if playerJSON['data'][i]['leaderboard']['difficulty'][
                'status'] == 3:  # Checks if the difficulty is ranked. discords qualified maps
                if playerJSON['data'][i]['leaderboard']['difficulty'][
                    'modeName'] == 'Standard':  # Filters out only standard maps/difficulties
